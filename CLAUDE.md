...
- **TypeORM é o ORM decidido em 2026-09-17**, registrado na ADR `vitacare-decisao-orm-typeorm` do vault. Como o TypeORM não tem filtro global de tenant, **todo acesso a dados fora de `repositories/` passa por `TenantScopedRepository`**, que injeta a organização do contexto autenticado; `@InjectRepository` fora dali é bloqueado por regra de ESLint. A primeira migration e as entidades de plano, organização, usuário, acesso e auditoria existem. As decisões de identidade tomadas em 2026-09-18 estão em `2026-09-18-identidade-documentos-schema` no vault; fluxos de autenticação e retenção continuam pendentes. Migrations ficam em `src/database/migrations/` e as convenções estão em `src/database/README.md`. `synchronize: false` e `migrationsRun: false` são intencionais para o banco externo. Uma mudança de schema exige migration explícita e validação em ambiente controlado; não executar migration em base real por suposição.
...
- A API já tem prefixo/versão configuráveis, Swagger/OpenAPI, DTO de paginação, rate limit, Helmet, CORS por ambiente e logs estruturados. Preserve o pipeline em `src/app.setup.ts` e os contratos públicos ao acrescentar módulos.
- **Contrato de resposta (decidido em 2026-09-17):** sucesso sai como `{ data, meta }`, com `meta.requestId` e `meta.timestamp` sempre presentes e `meta.pagination`/`meta.message` quando houver. Erro sai como `{ error: { code, message, detail, fields }, meta }`, com `code` no formato `DOMINIO_MOTIVO`, `message` amigável em português e `detail` técnico escrito por nós — nunca stack, mensagem crua de driver ou valor de campo clínico. Em 500 o `detail` é `null`. Validação de campo responde **422**. `/health/live`, `/health/ready`, `/api/docs` e `/api/docs-json` ficam **fora** do envelope de propósito. Detalhes em `docs/api/README.md`.
- `/health/live` verifica apenas o processo; `/health/ready` verifica PostgreSQL e Redis. Não trocar essas semânticas.
- Há testes unitários e e2e da infraestrutura e da primeira migration em PostgreSQL descartável, mas **nenhum caso de uso ou endpoint de negócio está implementado**. Os módulos iniciais registram apenas entidades. Isso não comprova schema aplicado em ambiente externo, S3 configurado ou deploy. O TCC prevê Lightsail e S3; o checkout atual não entrega essas integrações.
...
- Identidade e acesso: quem cria a primeira organização e o administrador; descoberta do tenant no login com e-mail único por organização; herança Administrador/Profissional; matriz de ação, campo e vínculo; recuperação por link (UC02) ou código de 6 dígitos (Figura 32); validade, revogação e convite. CPF do usuário é obrigatório e único por organização no schema; sua validação completa no fluxo de cadastro ainda deve ser implementada.
...
O DER, MER de 19 tabelas e protótipos divergem em campos, FKs, nulabilidade e tipos. Use `09-sistemas/vitacare/dados/vitacare-comparacao-der-mer.md` e `vitacare-integridade-dados.md` antes de cada próxima fatia de schema. Não copiar `DATETIME`, `ENUM` vazio ou `INT` para dinheiro, dose, referência e timestamps diretamente para PostgreSQL. Registre a decisão física e teste constraints de tenant e relacionamentos.

**Convenção de exclusão lógica definida por Mateus em 2026-09-18:** toda tabela nova e existente deve ter `deleted_at timestamptz` anulável, mapeado como `deletedAt` na entidade. Não usar `deactivated_at`. Nas tabelas atuais de organizações e usuários, `deleted_at` substitui o campo antigo e continua associado ao status `inactive` pela constraint; nas demais, a coluna começa nula. A coluna sozinha não cria filtro automático, política de retenção ou exclusão em cascata: cada caso de uso futuro deve definir leitura, revogação e preservação de histórico. Registrar esta convenção nas migrations, entidades, testes e no vault.

A CLI TypeORM em TypeScript deve resolver o alias `@/` das entidades por `tsconfig-paths/register`, conforme `package.json`; valide `migration:show`, `migration:run` e `migration:revert` em PostgreSQL descartável. A migration de produção usa os arquivos compilados em `dist/`.
...
Organize funcionalidades por domínio em `src/modules/<Dominio>/`, seguindo as convenções fechadas em 2026-09-17 e detalhadas em **`src/modules/README.md`** — leia esse arquivo antes de criar um módulo. Em resumo: módulo em PascalCase singular (`Patient`, `Auth`), uma pasta por caso de uso em PascalCase (`CreatePatient`, `ListPatients`), arquivos em camelCase com sufixo de responsabilidade (`createPatient.useCase.ts`, `.dto.ts`, `.type.ts`, `.parser.ts`, `.query.ts`, `.errors.ts`). Arquivo vira pasta quando passa de um. Controller fica na raiz do módulo; a rota permanece no plural.

Controllers cuidam de HTTP/DTO/status/Swagger; use cases cuidam das regras; services só quando forem uma camada útil de orquestração ou integração. **Entidade nunca sai pela API**: toda resposta passa por um parser que recebe o contexto, porque é ali que a visão simplificada do familiar é aplicada. **`try/catch` só nas bordas de I/O** (repositório e integrações), onde o erro é traduzível; o use case lança `DomainException` tipada com código do catálogo `<modulo>.errors.ts` e deixa subir. Toda rota declara `@ApiEnvelope(Dto)` e `@ApiErrors(...)`, senão o OpenAPI publica o DTO sem o envelope. Registre entidades via `TypeOrmModule.forFeature([...])` no módulo; não acesse repository diretamente do controller. Mantenha imports compartilhados pelo alias `@/` já usado no projeto.

**Convenção de testes alterada por Mateus em 2026-09-22:** nenhum `*.spec.ts` fica em `src/`. Todo teste vive em `tests/`, na raiz, com uma pasta por área de `src/` (`tests/common/`, `tests/database/`, `tests/health/`, `tests/redis/`, `tests/app/`, `tests/modules/<Dominio>/`) e arquivos planos dentro dela — a pasta por caso de uso existe apenas em `src/`. O sufixo separa as camadas: `*.e2e-spec.ts` roda por `npm run test:e2e` (config em `tests/jest-e2e.json`), o resto por `npm test`. Teste importa produção sempre pelo alias `@/`, nunca por caminho relativo a `src/`, e teste que exige PostgreSQL descartável fica atrás de `VITACARE_TEST_DATABASE_URL` e pula sem ela.
