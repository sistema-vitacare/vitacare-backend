...
4. **Exemplo completo:** comando `curl` copiável com dados sintéticos e um JSON de request válido; para GET sem body, não criar body fictício. Mostre resposta de sucesso com status e JSON completo, e tabela de **todas** as propriedades retornadas, inclusive campos aninhados. Para download/HTML, informe tipo de conteúdo e formato real, sem inventar JSON.
5. **Erros:** liste cada status que o fluxo pode produzir, condição e exemplo JSON do formato real. Explique validação de campos, conflito, ausência, falta de permissão, expiração, falha de serviço e comportamento idempotente quando pertinentes. Marque exemplos ilustrativos que não tenham teste/contrato fechado.
6. **Coleções e datas:** paginação, filtros, ordenação, cursor se houver, fuso, formato de instante e unidade clínica/monetária. Distinga ausência, `null`, `0` e `false`.
7. **Rastreabilidade:** paths do controller, DTO, use case, teste HTTP/integração, migration e nota de tarefa/evolução no vault. Registre data da última conferência com código e OpenAPI. Se o contrato mudou, cite a nota de evolução que explica antes/depois.
