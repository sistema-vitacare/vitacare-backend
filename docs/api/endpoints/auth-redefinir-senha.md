...
Controller `src/modules/Auth/auth.controller.ts`; DTO `src/modules/Auth/ResetPassword/resetPassword.dto.ts`; caso de uso `resetPassword.useCase.ts`; transação `src/modules/Auth/repositories/passwordReset.repository.ts`. Testes: `resetPassword.useCase.spec.ts`, `test/auth.e2e-spec.ts` (201 com `data: null`) e `test/authSchema.e2e-spec.ts` (uso único, sessões derrubadas e link anterior invalidado, em PostgreSQL real). Conferido contra o OpenAPI gerado em 2026-09-22. Vault: [[2026-09-21-autenticacao-sessoes-recuperacao]].
